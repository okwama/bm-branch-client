...
const StatCards: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        // Filter requests for the current branch
        const userUnscheduled = unscheduled.filter(request => request.branchId === user.id);
        const userPending = pending.filter(request => request.branchId === user.id);
        const userInTransit = inTransit.filter(request => request.branchId === user.id);
  ...
  return (
    <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div onClick={() => navigate('/dashboard/runs')} className="cursor-pointer">
        <StatCard 
          position={4} 
          title="Completed" 
