...
const RequestFormModal: React.FC<RequestFormModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    const fetchServiceCharges = async () => {
      ...
      try {
        const selectedBranch = branches.find(b => b.id.toString() === formData.branch_id.toString());
  ...
  useEffect(() => {
    if (isOpen && user?.id) {
      setFormData(prev => ({ ...prev, branch_id: user.id }));
    }
  }, [isOpen, user?.id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const selectedServiceCharge = serviceCharges.find(
        sc => sc.service_type_id.toString() === formData.service_type_id.toString()
      );
      ...
      const requestData: RequestData = {
        userId: user.id,
        userName: user.name,
        serviceTypeId: Number(formData.service_type_id),
